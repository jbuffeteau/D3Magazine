...
public class FrmMenu extends JFrame
{
    ...
    public FrmMenu()
    {
        ...
        // Remplir le JTABLE des magazines
        mdl =new ModelJTable();
        mdl.loadDatasMagazines(mesMagazines);
        tblMagazines.setModel(mdl);

        // Remplir la COMBOBOX des pigistes
        for(Pigiste pig : mesPigistes)
        {
            cboPigistes.addItem(pig.getNomPigiste());
        }
        ...
        tblMagazines.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                {
                    if(mag.getNumMagazine() == numMagazine)
                    {
                        mdl =new ModelJTable();
                        mdl.loadDatasArticle(mag.getLesArticles());
                        tblArticles.setModel(mdl);
                        txtMontantMagazine.setText(String.valueOf(mag.CalculerMontantMagazine()));
                        magazineSelectionne = mag;
                        break;
                    }
                }
            }
        ...
        tblArticles.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                String titreArticle = tblArticles.getValueAt(tblArticles.getSelectedRow(),0).toString();
                for(Article art : magazineSelectionne.getLesArticles())
                {
                    if(art.getTitreArticle().equals(titreArticle))
                    {
        ...
        btnAjouterArticle.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                {
                    // Il faut rechercher le pigiste dans la liste de tous les pigistes
                    for(Pigiste pig : mesPigistes)
                    {
                        if(pig.getNomPigiste().equals(cboPigistes.getSelectedItem().toString()))
                        {
                            Article art = new Article(txtTitreArticle.getText(),sldNbPages.getValue(),pig);
                            magazineSelectionne.AjouterArticle(art);
                            mdl =new ModelJTable();
                            mdl.loadDatasArticle(magazineSelectionne.getLesArticles());
                            tblArticles.setModel(mdl);
                            txtMontantMagazine.setText(String.valueOf(magazineSelectionne.CalculerMontantMagazine()));
                            //Pigiste lePigiste = new Pigiste(pig.getNumPigiste(),pig)
                        }
                    }
                }
            }
